"""Test harness para simular SistemaEolico con datos meteorológicos sintéticos.

Usa dos bucles anidados (multi-rate co-simulation):
  - Bucle externo: actualiza Ws cada 500 ms desde el CSV
  - Bucle interno: ejecuta 500 pasos de 1 ms del modelo físico

Uso:
    python Mockdata/test_harness_wind.py
    python Mockdata/test_harness_wind.py --full-24h   # simulación completa
"""
...
def cargar_o_generar(ruta_csv="datos_meteorologicos_eolicos_sinteticos.csv"):
    try:
        ...
        from Mockdata.deepseek_data_wind import df
        return df
...
def validar(resultados, df):
    ...
    # Correlación Ws vs Pw (debería ser alta: más viento → más potencia)
    corr = np.corrcoef(ws, p_inj)[0, 1]
    checks.append(("Correlacion Ws vs Pw > 0.8", corr > 0.8, f"{corr:.3f}"))
    ...
    # Ws sin negativos
    ws_min = ws.min()
    checks.append(("Ws sin negativos", ws_min >= 0, f"{ws_min:.1f} m/s"))
